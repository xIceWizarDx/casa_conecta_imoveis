...
export default function Painel() {
    ...
    const imageOptions = useMemo(() => images.map((img) => ({ value: img.id, label: `${img.id} — ${img.original_name}` })), [images]);
    ...
    return (
        <div className="old-site">
            ...
            <div className="container-responsive section-spacing pt-24 flex flex-col gap-6">
                {notice && (
                ...
                <div className="mb-4 border-b">
                    <div className="flex gap-2">
                        <button type="button" onClick={() => setTab('publicacoes')} className={`px-4 py-2 -mb-px border-b-2 ${tab === 'publicacoes' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>Publicações</button>
                        <button type="button" onClick={() => setTab('images')} className={`px-4 py-2 -mb-px border-b-2 ${tab === 'images' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>Imagens</button>
                    </div>
                </div>
                {/* Imagens */}
                {tab === 'images' && (
                                ...
                                {images.map((img) => (
                                    <div key={img.id} className="group relative overflow-hidden rounded-md border">
                                        ...
                                        <div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/50 p-1 text-xs text-white opacity-0 transition group-hover:opacity-100">
                                            <span className="truncate" title={img.original_name}>{img.original_name}</span>
                    ...
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <Button variant="secondary" onClick={refreshSlides} disabled={slidesLoading}>Atualizar</Button>
                            </div>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                            <div>
                                ...
                                <select
                                    ...
                                    value={newSlide.image_id ?? ''}
                                    onChange={(e) => setNewSlide((s) => ({ ...s, image_id: Number(e.target.value) }))}
                                >
                                ...
                                <div className="mt-2 flex items-center gap-2">
                                    <Dialog open={imagePickerOpen && imagePickerFor === 'featured'} onOpenChange={(o) => { setImagePickerOpen(o); if (!o) setImagePickerFor(null); }}>
                                        <DialogTrigger asChild>
                                            <Button type="button" variant="secondary" onClick={() => { setImagePickerFor('featured'); setImagePickerOpen(true); }}>Selecionar da galeria</Button>
                                        </DialogTrigger>
                                        <DialogContent className="sm:max-w-3xl">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                                                {images.map((img) => (
                                                    <button key={img.id} type="button" className="overflow-hidden rounded-md border focus:outline-none focus:ring-2 focus:ring-ring" onClick={() => { setNewFeatured((s) => ({ ...s, image_id: img.id })); setImagePickerOpen(false); setImagePickerFor(null); }}>
                                    ...
                                    {newFeatured.image_id && (
                                        <img src={images.find((i) => i.id === newFeatured.image_id)?.url} alt="Selecionada" className="h-10 w-14 rounded object-cover" />
                                    )}
                                </div>
                                <div className="mt-2 flex items-center gap-2">
                                    <Dialog open={imagePickerOpen && imagePickerFor === 'slide'} onOpenChange={(o) => { setImagePickerOpen(o); if (!o) setImagePickerFor(null); }}>
                                        <DialogTrigger asChild>
                                            <Button type="button" variant="secondary" onClick={() => { setImagePickerFor('slide'); setImagePickerOpen(true); }}>Selecionar da galeria</Button>
                                        </DialogTrigger>
                                        <DialogContent className="sm:max-w-3xl">
                                            <DialogHeader>
                                                <DialogTitle>Escolher imagem</DialogTitle>
                                            ...
                                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                                                {images.map((img) => (
                                                    <button key={img.id} type="button" className="overflow-hidden rounded-md border focus:outline-none focus:ring-2 focus:ring-ring" onClick={() => { setNewSlide((s) => ({ ...s, image_id: img.id })); setImagePickerOpen(false); setImagePickerFor(null); }}>
                                                        <img src={img.url} alt={img.original_name} className="h-28 w-full object-cover" />
                                                    </button>
                                                ))}
                                            </div>
                                    ...
                                    {newSlide.image_id && (
                                        <img src={images.find((i) => i.id === newSlide.image_id)?.url} alt="Selecionada" className="h-10 w-14 rounded object-cover" />
                                    )}
                                </div>
                            </div>
                            <div>
                                <Label>Título</Label>
                                <Input value={newSlide.title ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, title: e.target.value }))} placeholder="Ex: Casa moderna no Setor Marista" />
                            </div>
                            <div>
                                <Label>Preço</Label>
                                <Input value={newSlide.price ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, price: formatCurrencyBRLInput(e.target.value) }))} inputMode="numeric" placeholder="Ex: R$ 1.250.000,00" />
                            </div>
                        ...
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
                            <div>
                                ...
                                <Input value={newSlide.subtitle ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, subtitle: e.target.value }))} placeholder="Ex: Próximo ao Flamboyant" />
                            </div>
                            <div>
                                <Label>Quartos</Label>
                                <Input type="number" min={0} value={newSlide.bedrooms ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, bedrooms: Number(e.target.value || 0) }))} inputMode="numeric" placeholder="Ex: 3" />
                            </div>
                            <div>
                                <Label>Banheiros</Label>
                                <Input type="number" min={0} value={newSlide.bathrooms ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, bathrooms: Number(e.target.value || 0) }))} inputMode="numeric" placeholder="Ex: 2" />
                            </div>
                            <div>
                                <Label>Área</Label>
                                <Input value={newSlide.area ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, area: formatAreaM2Input(e.target.value) }))} inputMode="numeric" placeholder="Ex: 120 m²" />
                            </div>
                            <div>
                                <Label>Bairro</Label>
                                <Input value={newSlide.neighborhood ?? ''} onChange={(e) => setNewSlide((s) => ({ ...s, neighborhood: e.target.value }))} placeholder="Ex: Setor Marista" />
                            </div>
                        </div>
                        <div className="flex items-center gap-6">
                            <label className="flex items-center gap-2 text-sm">
                                <input type="checkbox" checked={!!newSlide.is_new} onChange={(e) => setNewSlide((s) => ({ ...s, is_new: e.target.checked }))} />
                                Novo
                            </label>
                            <label className="flex items-center gap-2 text-sm">
                                <input type="checkbox" checked={!!newSlide.is_published} onChange={(e) => setNewSlide((s) => ({ ...s, is_published: e.target.checked }))} />
                                Publicado
                            </label>
                        </div>
                                ...
                                <div key={s.id} className="overflow-hidden rounded-md border">
                                    ...
                                    <div className="flex items-center justify-between gap-2 p-2">
                                        ...
                                        <div className="flex shrink-0 items-center gap-1">
                                            ...
                                            <Button variant={s.is_published ? 'default' : 'secondary'} onClick={() => toggleSlidePublish(s.id)} title="Publicar/Despublicar">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
                            <div>
                                ...
                                <select
                                    className="mt-1 w-full rounded-md border bg-background p-2"
                                    value={newFeatured.image_id ?? ''}
                                    onChange={(e) => setNewFeatured((s) => ({ ...s, image_id: Number(e.target.value) }))}
                                >
                                    <option value="" disabled>
                                        Selecione…
                                    </option>
                            ...
                            <div>
                                <Label>Título</Label>
                                <Input value={newFeatured.title ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, title: e.target.value }))} placeholder="Ex: Apartamento premium Jardim Goiás" />
                            </div>
                            <div>
                                <Label>Bairro</Label>
                                <Input value={newFeatured.neighborhood ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, neighborhood: e.target.value }))} placeholder="Ex: Jardim Goiás" />
                            </div>
                            <div>
                                <Label>Preço</Label>
                                <Input value={newFeatured.price ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, price: formatCurrencyBRLInput(e.target.value) }))} inputMode="numeric" placeholder="Ex: R$ 950.000,00" />
                            </div>
                            <div className="flex items-end gap-2">
                        ...
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-6">
                            <div>
                                <Label>Quartos</Label>
                                <Input type="number" min={0} value={newFeatured.bedrooms ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, bedrooms: Number(e.target.value || 0) }))} inputMode="numeric" placeholder="Ex: 3" />
                            </div>
                            <div>
                                <Label>Banheiros</Label>
                                <Input type="number" min={0} value={newFeatured.bathrooms ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, bathrooms: Number(e.target.value || 0) }))} inputMode="numeric" placeholder="Ex: 2" />
                            </div>
                            <div>
                                <Label>Área</Label>
                                <Input value={newFeatured.area ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, area: formatAreaM2Input(e.target.value) }))} inputMode="numeric" placeholder="Ex: 120 m²" />
                            </div>
                            <div>
                                <Label>Tipo</Label>
                                <select
                            ...
                            <div>
                                ...
                                <Input value={newFeatured.price_range ?? ''} onChange={(e) => setNewFeatured((s) => ({ ...s, price_range: e.target.value }))} placeholder="Ex: 800000-1200000" />
                            </div>
                            <div>
                                <Label>Recém lançado</Label>
                                <div className="mt-2 flex items-center gap-4">
                                    <label className="flex items-center gap-2 text-sm">
                                        <input type="checkbox" checked={!!newFeatured.is_new} onChange={(e) => setNewFeatured((s) => ({ ...s, is_new: e.target.checked }))} />
                                        Novo
                                    </label>
                                    <label className="flex items-center gap-2 text-sm">
                                        <input type="checkbox" checked={!!newFeatured.is_published} onChange={(e) => setNewFeatured((s) => ({ ...s, is_published: e.target.checked }))} />
                                        Publicado
                                    </label>
                                </div>
                            </div>
                                ...
                                <div key={f.id} className="overflow-hidden rounded-md border">
                                    ...
                                    <div className="flex items-center justify-between gap-2 p-2">
                                        ...
                                        <div className="flex shrink-0 items-center gap-1">
                                            ...
                                            <Button variant={f.is_published ? 'default' : 'secondary'} onClick={() => toggleFeaturedPublish(f.id)} title="Publicar/Despublicar">
